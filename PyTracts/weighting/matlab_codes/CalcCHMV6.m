function CalcCHMV6(fname)
tic
grad_dirs=[]; vec=[]; rDWI=[];
load('AxCal3Ddata','bval', 'mask', 'grad_dirs', 'MD1000', 'phi_q','theta_q', 'R_q', 'rDWI','Param','vec', 'MD250','DT1000');
pfr=zeros(size(mask));
ph=zeros(size(mask));
...
for i=1:length(zlocs)
    ...
    CMDcsf(xlocs(i), ylocs(i), zlocs(i))=prcsf(xlocs(i), ylocs(i), zlocs(i));
...
i1.hdr.dime.scl_slope=1/100;
save_untouch_nii(i1, [fname,'_',num2str(al),'_', num2str(be),'_CMDr5.nii']);
i1.img=CMDfh.*100;
save_untouch_nii(i1, [fname,'_',num2str(al),'_', num2str(be),'_CMDh5.nii']);
i1.img=CMDcsf.*100;
save_untouch_nii(i1, [fname,'_',num2str(al),'_', num2str(be),'_CMDcsf5.nii']);
...
i1.hdr.dime.bitpix=64;
i1.hdr.dime.scl_slope=1/100;
save_untouch_nii(i1, [fname,'_',num2str(al),'_', num2str(be),'_AxFr5.nii']);
i1.img=ph.*100;
save_untouch_nii(i1, [fname,'_',num2str(al),'_', num2str(be),'_AxFh5.nii']);
i1.img=pcsf.*100;
save_untouch_nii(i1, [fname,'_',num2str(al),'_', num2str(be),'_AxCSF5.nii']);
i1.img=pasi;
i1.hdr.dime.scl_slope=1;
save_untouch_nii(i1, [fname,'_',num2str(al),'_', num2str(be),'_AxPasi5.nii']);
