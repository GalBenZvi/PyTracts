...
def load_dwi_files(folder_name: Path, small_delta=15.5):
    """
    param:
        folder_name -
        small_delta -
    ...
    for file in dwi_folder.iterdir():
        if file.suffix == ".bvec":
            ...
            dwi_file = file
    labels_file_name = folder_name / "anat" / "prep.anat" / "T1_fast_mixeltype.nii.gz"
    labels_img, hardi_img = resample_to_dwi(labels_file_name, dwi_file)
    bval_file = bvec_file.with_suffix(".bval")
    data = hardi_img.get_fdata()
    affine = hardi_img.affine
    gtab = gradient_table(str(bval_file), str(bvec_file), small_delta=small_delta)
    # labels_img = nib.load(labels_file_name)
    labels = labels_img.get_fdata()
    white_matter = labels == 2  # | (labels == 2)  # 3-WM, 2-GM

    return gtab, data, affine, labels, white_matter, dwi_file, bvec_file
...
def nodes_by_index(folder_name):
    ...
    lab = folder_name / "highres_atlas2dwi.nii.gz"
    # lab = os.path.join(folder_name, r"HighresAtlas2Highres_resampled.nii.gz")
    lab_file = nib.load(lab)
    lab_labels = lab_file.get_data()
...
def load_weight_by_img(bvec_file, weight_by):
    ...
    weight_by_file = glob.glob(f"{Path(bvec_file).parent}/*{weight_by}.nii")[0]
